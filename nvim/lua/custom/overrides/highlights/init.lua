...
local function merge_table(integration, orign_table)
  return vim.tbl_extend("force", integration, orign_table)
end

local integrations = {
  navic = require "custom.overrides.highlights.integrations.navic",
  git_graph = require "custom.overrides.highlights.integrations.git-graph",
  fugit2 = require "custom.overrides.highlights.integrations.fugit2",
  nvimtree = require "custom.overrides.highlights.integrations.nvimtree",
  nvim_bqf = require "custom.overrides.highlights.integrations.nvim-bqf",
  vm_visual_multi = require "custom.overrides.highlights.integrations.vim-visual-multi",
  nui = require "custom.overrides.highlights.integrations.nui",
}
...
M.override = merge_table(override, M.override)

-- navic
M.override = merge_table(integrations.navic, M.override)

-- nvimtree
M.override = merge_table(integrations.nvimtree, M.override)

-- git-graph
M.add = merge_table(integrations.git_graph, M.add)

-- fugit2
M.add = merge_table(integrations.fugit2, M.add)

-- nvim_bqf
M.add = merge_table(integrations.nvim_bqf, M.add)

-- vm_visual_multi
M.add = merge_table(integrations.vm_visual_multi, M.add)

-- nui
M.add = merge_table(integrations.nui, M.add)
